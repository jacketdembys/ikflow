...
class SymmetricFullCoupling(nn.Module):
    def __init__(self, dim, split_scheme, subnet_constructor, quat_start_idx=None):
        ...
        self.scale_limit = 1.0  # Fixed scale limit to prevent divergence
        self.dim = dim
    ...
    def forward(self, x, rev=False, use_autograd_logdet=True, use_trace_estimate=False):
        ...
        if not rev:
            for layer in self.layers:
                x, log_det = layer(x, rev=rev, use_autograd_logdet=use_autograd_logdet, use_trace_estimate=use_trace_estimate)
...
def get_model(architecture='symmetric_full', dim=2, split_scheme=[1, 1], n_blocks=4):
    if architecture not in coupling_layers:
        raise ValueError(f"Unknown architecture: {architecture}")
    model_class = coupling_layers[architecture]
    layers = [model_class(dim=dim, split_scheme=split_scheme, subnet_constructor=make_mlp) for _ in range(n_blocks)]
    return FlowModel(layers)

N_BLOCKS = 4

def train_on_two_moons(architecture='symmetric_full', save_path='trained_model.pth', logdet_type='autograd'):
    X, y = make_moons(n_samples=60000, noise=0.1, random_state=42)
    ...
    model = get_model(architecture=architecture, dim=2, split_scheme=[1, 1], n_blocks=N_BLOCKS)
    print(model)
    ...
    for epoch in range(1, 101):
        ...
        for batch, in train_loader:
            ...
            epoch_logpz.append(log_pz.mean().item())
            epoch_logdet.append(logdet.mean().item())
        ...
        logpz_history.append(np.mean(epoch_logpz))
        logdet_history.append(np.mean(epoch_logdet))
...
def visualize_two_moons_model(architecture='symmetric_full', model_path='trained_model.pth', logdet_type='autograd'):
    ...
    if os.path.exists(model_path):
        ...
        # Removed confusion_matrix as it's a self-comparison and not informative
        ...
        # Plot histograms of latent variables z
        plt.figure(figsize=(10, 4))
        for i in range(z.shape[1]):
            plt.subplot(1, z.shape[1], i+1)
            plt.hist(z[:, i].detach().numpy(), bins=30, alpha=0.7, color='gray', edgecolor='black')
            plt.title(f'z dim {i}')
        plt.tight_layout()
        plt.savefig(f"z_hist_{architecture}_{logdet_type}.pdf")
        print(f"Latent histogram saved to z_hist_{architecture}_{logdet_type}.pdf")
        plt.show()
    else:
        print(f"Model path {model_path} does not exist.")

    # Option to sample from standard normal and invert
    SAMPLE_FROM_LATENT = True
    if SAMPLE_FROM_LATENT:
        with torch.no_grad():
            z_sample = torch.randn_like(z)
            x_sampled, _ = model(z_sample, rev=True, use_autograd_logdet=(logdet_type=='autograd'), use_trace_estimate=(logdet_type=='trace'))

        plt.figure(figsize=(6, 5))
        plt.scatter(x_sampled[:, 0], x_sampled[:, 1], alpha=0.5, color='green', s=10)
        plt.title("Samples from N(0, I) mapped to data space")
        plt.tight_layout()
        sampled_fig_path = f"z_sample_to_data_{architecture}_{logdet_type}.pdf"
        plt.savefig(sampled_fig_path)
        print(f"Generated samples saved to {sampled_fig_path}")
        plt.show()


if __name__ == '__main__':
    architectures_to_train = ['realnvp'] # ['symmetric_full', 'realnvp']
