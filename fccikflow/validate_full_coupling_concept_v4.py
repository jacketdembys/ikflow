...
class SymmetricFullCoupling(nn.Module):
    def __init__(self, dim, split_scheme):
        super().__init__()
    ...
    def forward(self, x, rev=False):
        ...
        x1, x2 = x[:, :self.d1].contiguous(), x[:, self.d1:].contiguous()
        if not rev:
            s2 = torch.tanh(self.s1(x2)) * self.scale_limit
            t2 = self.t1(x2)
            y1 = x1 * torch.exp(s2) + t2
            s1 = torch.tanh(self.s2(y1)) * self.scale_limit
            t1 = self.t2(y1)
            y2 = x2 * torch.exp(s1) + t1
            y = torch.cat([y1, y2], dim=1)
            log_det = s2.sum(dim=1) + s1.sum(dim=1)
        else:
            s1 = torch.tanh(self.s2(x1)) * self.scale_limit
            t1 = self.t2(x1)
