...
class DiffIKDataset(Dataset):
    def __init__(self, filename_D, filename_Q):
        self.q = torch.load(filename_Q, weights_only=False)          # shape: [N, dof]
        self.pose = torch.load(filename_D, weights_only=False)     # shape: [N, 6 or 7]
...
class DiffIKDenoiser(nn.Module):
    def __init__(self, dof=7, pose_dim=7, hidden_dim=256, time_embed_dim=64):
        super().__init__()
        ...
        self.net = nn.Sequential(
            nn.Linear(dof + pose_dim + time_embed_dim, hidden_dim),
            nn.ReLU(),
    ...
    def forward(self, q_t, pose, t):
        # t: [B,] → [B,1] → [B, time_embed_dim]
        t_embed = self.time_embed(t.unsqueeze(-1).float())
        x = torch.cat([q_t, pose, t_embed], dim=-1)  # [B, dof + pose_dim + t_emb]
        return self.net(x)  # output ε̂ (same shape as q_t)
"""
...
class DiffIKLightningModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_pos_err_mm", l2_errors.mean() * 1000.0, prog_bar=True)
        self.log("val_ori_err_deg", ang_errors.mean() * (180.0 / 3.14159), prog_bar=True)

        val_pose_err = l2_errors.mean()*1000 + ang_errors.mean()* (180.0 / 3.14159)
        self.log("val_pose_err", l2_errors.mean()*1000 + ang_errors.mean()* (180.0 / 3.14159))
